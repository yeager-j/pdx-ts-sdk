...
failure modes. `@pdx-ts/sdk` declaration-merges against it
...
time when the package is installed and imported, and degrades to an unchecked `string` per registry
otherwise — this is the same optional-package pattern as the rest of the SDK's vanilla surface,
