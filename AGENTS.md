...
underneath it; `packages/codegen` and `packages/vanilla-codegen` are the two private generators;
`packages/stellaris-vanilla` is the install-derived identifier package. Every gate below runs from
the repository root.
...
`packages/sdk/src/generated/` is committed output from `packages/codegen/` (`@pdx-ts/codegen`).
Never edit it by hand.

The main inputs are:

- `vendor/cwtools-stellaris-config/` for vendored CWT rules and Stellaris documentation dumps
- `packages/codegen/src/content-manifest.ts` for the content registries intentionally exposed by
  the SDK
- `packages/codegen/src/overlay.ts` for reviewed departures from a mechanical reading of the rules
- emitters and parsers under `packages/codegen/src/`
...
`packages/codegen/src/drift-baseline.json` only after reviewing and intentionally accepting drift between
the vendored rule sources and documentation dumps.

## Vanilla identifier package

`packages/stellaris-vanilla` (`@pdx-ts/stellaris-vanilla`) is a separate, install-derived package
...
generated by `@pdx-ts/vanilla-codegen` (`packages/vanilla-codegen`), a generator separate from
`@pdx-ts/codegen`: install-derived
...
`export * as vanilla`) is emitted by the existing `@pdx-ts/codegen`, from `CONTENT_MANIFEST` plus
`VANILLA_REF_EXTRAS` (`packages/codegen/src/content-manifest.ts`) — it is glue over the separate
package,
not the package itself, and never reads an install.

- `npm run vanilla:codegen` regenerates `packages/stellaris-vanilla/src` and its `package.json`
...
- Review the complete `packages/stellaris-vanilla/src` diff as a public-API change, the same way a
...
- Licensing boundary, enforced by the generator itself rather than left to convention: this package
  ...
  `packages/stellaris-vanilla/PROVENANCE.md`.
- `npm run vanilla:codegen:check` is maintainer-local, inherently install-gated — CI has no
...
2. Add the type and source file to the explicit allowlist in
   `packages/codegen/src/content-manifest.ts`,
