...
generated by `tools/vanilla-codegen`, a generator separate from `tools/codegen`: install-derived
...
`export * as vanilla`) is emitted by the existing `tools/codegen`, from `CONTENT_MANIFEST` plus
`VANILLA_REF_EXTRAS` (`tools/codegen/content-manifest.ts`) — it is glue over the separate package,
