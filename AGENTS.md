...
failure modes. `@pdx-ts/sdk` currently declaration-merges against it
(`packages/sdk/src/identifiers/contracts.ts`'s empty `VanillaIds`/`VanillaScriptedTriggers`/
`VanillaScriptedEffects`/`VanillaTries` interfaces) so a vanilla reference is checked at compile
time. The package is a hard peer dependency
([ADR-0006](docs/adr/0006-stellaris-ids-is-a-hard-dependency.md), reversing the optionality half of
ADR-0003): the unchecked-`string` degraded mode that tolerated its absence is decided out and is
removed with SDK-16 slice 2, after which the SDK imports the package's types directly. Projects
supply the game-version range below.
...
- The scopes are the one thing derived from a body rather than read off one, so the boundary there
  ...
  The SDK's side is `packages/sdk/src/script/scripted.ts`, which consumes the `VanillaScriptedTriggers` /
