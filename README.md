...
in game. `@pdx-ts/stellaris-ids` closes that gap: a version-pinned package
of every identifier a real install defines, generated by `@pdx-ts/codegen-vanilla`
...
`packages/sdk/src/generated/` is produced by `@pdx-ts/codegen-cwt` from the vendored
...
`packages/codegen-cwt/src/drift-baseline.json`; codegen fails when either set moves rather
...
audited file, `packages/codegen-cwt/src/overlay.ts`. What the rules cannot supply at all —
...
members of their own (`packages/codegen-cwt`, `packages/codegen-vanilla`), and the
