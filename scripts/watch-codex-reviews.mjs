 ...
 */
export function deriveCodexReviewState(pullRequest, issueComments, reviews, reviewComments) {
...
export function formatCodexReviewState(state) {
  ...
  if (state.status === "waiting") {
    return `${prefix} Waiting for Codex review to start.`;
  }
...
async function loadCodexReviewState(pullRequest) {
  ...
  const issueComments = await githubApiPages(
    `${base}/issues/${pullRequest.number}/comments?per_page=100`
  );
  const status = deriveCodexReviewState(pullRequest, issueComments, [], []);
  ...
  return deriveCodexReviewState(pullRequest, issueComments, reviews, reviewComments);
}
...
/**
 * Polls several pull requests, writes only changed states, and stops when all reviews complete.
...
export async function watchCodexReviews(pullRequests, options = {}) {
  ...
  while (true) {
    ...
    const changed = states.filter((state) => {
      ...
      const oldFingerprint = previous.get(state.pullRequest.label);
      previous.set(state.pullRequest.label, fingerprint);
      return fingerprint !== oldFingerprint;
    });
    if (changed.length > 0) {
      write(changed.map(formatCodexReviewState).join("\n\n"));
    }
    if (options.once === true || states.every((state) => state.status === "completed")) {
      return states;
    }
