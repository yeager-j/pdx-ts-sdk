...
**Landed 2026-08-02** (SDK-12). Separate install-derived package, version-pinned
to the game, closing the one place the SDK knowingly degrades cross-content
references to raw strings. Types only, per-registry segmentation, identifiers
never script bodies. Also carries sound and picture names, which
`decision.sound`, `event.picture`, and `event.show_sound` used to take as
`string`.

`@pdx-ts/stellaris-ids` is generated by `packages/codegen-vanilla` from a real
install and committed: 38 registries / 29,725 ids, 1,618 scripted triggers and
1,657 scripted effects with their `$PARAM$` lists, at npm version `4.4.6` —
the package version _is_ the game version, and `buildMod` refuses a mismatched
`stellaris.load()` install. The SDK side is `vanilla.*`
(`src/generated/vanilla-refs.ts`), one checked constructor per registry, plus a
navigable trie for the four registries too large for a completion menu
(`sprite`, `sound`, `sound_effect`, `static_modifier`). Tries come in two
shapes: `sprite`/`sound`/`sound_effect` nest by name, so the property path
rejoined with `_` _is_ the id; `static_modifier` nests by the vanilla file each
id is defined in, because its 3,081 names share first words by coincidence
(1,028 segment buckets, most holding one id) while the files Paradox groups
them into are real categories. With the package absent every helper accepts any
string, per registry rather than all-or-nothing.

The Record-key rewiring noted above (`Record<string, number>` for
`economic_category.amounts`, `civic_or_origin.leader_background_job_weight`,
`ship_size.min_upgrade_cost`) is now **unblocked but deliberately deferred**:
`VanillaId<K>` is exactly the key type those fields want. It is an emitter
change, so it needs its own corpus evidence — the shape-conformance gate has to
show that every key vanilla actually writes in those maps is an id the package
carries, before narrowing a key type that currently accepts anything.
...
> **Historical status — superseded by SDK-72, 2026-08-04.** The SDK-22 and
> SDK-23 entries below record the former pure-function/free-definer migration.
> Current authoring is capability-only: `createMod`, capability-owned methods,
> explicit `feature`, `discoverFeatures`, and `mod.compile`.
