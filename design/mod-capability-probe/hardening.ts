import type { defineHardening as defineOriginalHardening } from "../../examples/hardening/mod.ts";
import {
  countryFlags,
  eventTarget,
  hasAuthority,
...
export function defineCapabilityHardening(
  vanilla: VanillaView
): ReturnType<typeof defineOriginalHardening> {
  const mod = createMod(
    {
  ...
  const hardeningFlags = countryFlags("pdx_hardening_entry_fired");
  const hardeningTarget = eventTarget<"planet">("pdx_hardening_chain_planet");
  ...
  const markerBuilding = mod.building("marker", {
    ...
    planetModifier: (modifiers) => {
      modifiers.planet.jobs.society.research.produces.mult(0.01);
    },
  });
  ...
  const tradition = mod.tradition("marker", {
    ...
    modifier: (modifiers) => {
      modifiers.country.unity.produces.mult(0.01);
    },
  });
