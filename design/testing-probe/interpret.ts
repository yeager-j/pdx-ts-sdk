...
import { makeScope } from "../../packages/sdk/src/script/effects/recorder.ts";
...
function execCtxFor(scope: SimScope<SimScopeName>): ExecCtx {
  return { state: scope.state, root: scope.id, from: undefined, forcedArms: [] };
}
...
export function applyEffectEntries(
  ...
  for (const entry of entries) {
    ...
    throw new InterpreterError(
      known
        ? `Effect "${entry.key}" is real but unimplemented in the probe interpreter. ` +
            coverageSummary()
