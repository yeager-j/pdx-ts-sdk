/**
 * The interpreter's whitelist — this spike's overlay. One audited table;
...
export const EFFECT_SEMANTICS: Readonly<Record<string, EffectImpl>> = {
  ...
  add_resource: {
    ...
    apply: (entry, scope, ex) => {
      ...
      for (const field of blockEntries(entry)) {
        if (field.key === "mult") {
          throw new InterpreterError(
            `add_resource with mult is not modeled by the probe interpreter. ${coverageSummary()}`
          );
        }
  ...
  save_event_target_as: {
    note: "Targets are a name -> entity map; a later save overwrites, as in game. Lifetime is world-long in this probe (game scopes them to the event chain).",
    apply: (entry, scope, ex) => {
      ex.state.targets.set(stringArg(entry), scope);
    },
  },
  log: {
...
export function resolveEventTarget(name: string, ex: ExecCtx): EntityId {
  const target = ex.state.targets.get(name);
  if (target === undefined) {
    throw new InterpreterError(
