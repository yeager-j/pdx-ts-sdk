 ...
 * delivers queued fires due in the window in timestamp order (FIFO within a
...
export class World {
   ...
   * window in (dueDay, enqueue-order) order; delivered immediates may
  ...
  advance(days: number): void {
    ...
    for (;;) {
      const next = this.state.queue
        ...
        .sort((a, b) => a.dueDay - b.dueDay || a.seq - b.seq)[0];
      if (next === undefined) {
        break;
      }
