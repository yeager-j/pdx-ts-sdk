...
export function planPatchEmission(args: {
  ...
  const ruleSummary =
    row.repeat.state === "verified"
      ? `Overrides verified against Stellaris ${row.verifiedAgainst}. Registry \`${registry}\` is last-wins`
      : `Override planned against Stellaris ${row.verifiedAgainst}. Registry \`${registry}\` uses an ASSUMED last-wins rule`;

  const header = [
    `# Generated by @pdx-ts/sdk -- do not edit.`,
    `# ${ruleSummary}: this file byte-sorts after every file defining each key below.`,
