...
import type { SpecimenDef } from "./specimen.ts";
...
export interface IdProfile {
  ...
  /**
   * The segment inserted between the mod prefix and a specimen's logical name.
   * Override it when this registry needs a different id convention.
   */
...
export interface ContentCapabilityMethods<P extends string, I extends IdProfile> {
  ...
  /**
   * Defines a specimen from its logical name.
   * The capability mints and owns the full id; the returned branded reference
   * flows into matching content-reference fields.
   */
  specimen<const Name extends string>(
    name: Name,
    def: Omit<SpecimenDef<MintedContentId<P, I, "specimen", Name>>, "id">
  ): ContentItem<"specimen", SpecimenDef<MintedContentId<P, I, "specimen", Name>>>;
  /**
   * Mints a specimen id without its definition.
   * Define it later with its `define(...)` method when a cycle needs the id first —
   * a specimen that names itself, or two that name each other.
   * The handle is a reference, not content: place the item `define(...)` returns.
   */
...
export function contentCapabilityMethods<P extends string, I extends IdProfile>(
  ...
  return Object.freeze({
    ...
    specimenHandle: <const Name extends string>(name: Name) => {
      return createContentHandle(
        "specimen",
        mint("specimen", name),
        (def: SpecimenDef<MintedContentId<P, I, "specimen", Name>>) => {
