...
import { SPECIMEN_LOCALISATION, type SpecimenDef } from "./specimen.ts";
...
/** What a specimen feature can contain. */
export type SpecimenItem = ContentItem<"specimen", SpecimenDef>;

/**
 * Internal lowering primitive for a specimen. Public authors call
 * `mod.specimen(name, def)`, then place the returned item with
 * `mod.feature(...)` before compiling the same capability.
 */
export function defineSpecimen<const Id extends string>(
  def: SpecimenDef<Id>
): ContentItem<"specimen", SpecimenDef<Id>> {
  return {
    itemKind: "content",
    type: "specimen",
    id: def.id,
