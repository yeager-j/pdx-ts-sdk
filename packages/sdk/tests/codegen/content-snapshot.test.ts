...
describe("content-type codegen", () => {
  ...
  it("asserts an arity the rules get wrong rather than declining the field", () => {
    ...
    // measures the arity against the corpus instead, so the field is authorable
    // and the refusal list is empty — a defect that is measured does not need a
    // reviewer to vouch for it.
    const job = emissions.get("job")!;
    ...
    expect([...CONTENT_DECLINED_FIELDS]).toEqual([]);
  });
