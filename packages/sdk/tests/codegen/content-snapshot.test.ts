...
describe("content-type codegen", () => {
  it("generates relic, mission, and mission-category authoring surfaces", () => {
    ...
    expect(mission?.code).not.toContain("potentialOperator?:");
    expect(mission?.code).not.toContain("onAccept?:");
    expect(missionCategory?.code).toContain("isContract: false;");
  });
  ...
  it("asserts an arity the rules get wrong rather than declining the field", () => {
    ...
    // member cannot represent. SDK-294 separately refuses the contract-only
    // mission surface because its dynamic location scopes are not yet modeled.
