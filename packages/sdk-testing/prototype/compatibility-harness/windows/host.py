...
def game_windows(desktop, pid):
    ...
    def visit(window, parameter):
        owner = w.DWORD()
        window_pid(window,c.byref(owner))
        if owner.value == pid:
            found.append({'window':window,'visibleStyle':bool(window_visible(window))})
        return True
...
desktop_name = 'sdk448_' + config['runId'].replace('-','')
desktop = job = None
process = Process()
samples = []
...
try:
    ...
    os.environ['SteamAppId'], os.environ['SDK448_NATIVE'] = '281990',str(native)
