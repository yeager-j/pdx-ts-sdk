...
const executableSha256 = "bc451c72d9654c8901f1bb0bee1dd78d76f415465c2fbf746e9f98ade333173a";
type Configuration = {
  game: string;
  python: string;
...
export function create(context: Context): Adapter {
  ...
  return {
    async launch(control) {
      ...
      const saveDirectory = join(profile, "save games/sdk449");
      ...
      json(join(profile, "continue_game.json"), {
        title: "save games/sdk449/baseline",
        desc: "Windows compatibility spike",
        date: "2200.01.01",
      });
      const mod = join(profile, "mod/sdk449");
      ...
      writeFileSync(
        join(profile, "mod/sdk449.mod"),
        `name="SDK449 Windows spike"\npath="${mod.replaceAll("\\", "/")}"\nsupported_version="4.4.*"\n`
      );
      const dlcRoot = join(dirname(config.game), "dlc");
      const dlcDefinitions = readdirSync(dlcRoot, { recursive: true, withFileTypes: true })
      ...
      const dependencies = {
        ...
        dlcDefinitions: Object.fromEntries(
          dlcDefinitions.map((name) => [name, hash(join(dirname(config.game), name))])
        ),
      };
      ...
      await until(() => {
        ...
        if (existsSync(join(native, "bridge-failure.json")))
          throw new Error(readFileSync(join(native, "bridge-failure.json"), "utf8"));
      ...
      const metadata: Metadata = {
        ...
        environment: {
          ...
          game: "Pegasus 4.4.6 (fdde), Steam build 24109497",
          executableSha256,
        ...
        fixture: {
          ...
          dependencies: [
            {
              name: "ordered-mod-settings-and-enabled-dlc-definitions",
