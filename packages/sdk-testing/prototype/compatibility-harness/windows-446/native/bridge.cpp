// Throwaway SDK449 bridge for the pinned Windows x64 Pegasus 4.4.6 executable.
...
static std::atomic<DWORD> nativeFault;
...
struct NativeString {
    ...
    explicit NativeString(const std::string& text) { engine<void(*)(void*,const char*)>(0x1ca8260)(bytes,text.c_str()); }
    ~NativeString() { engine<void(*)(void*)>(0x160670)(bytes); }
};
static Byte* lookup(const std::string& kind,unsigned id) {
    auto database=global(kind=="country"?0x3287788:0x3287328);
...
static Json snapshot() {
    ...
    return {{"world",world},{"day",date/24},{"paused",read<Byte>(idler,0x594)!=0},{"ai",read<Byte>(reinterpret_cast<void*>(base+0x299fc12))?"on":"off"},{"player",subject("country",human)}};
}
static Json targets() {
    auto state=global(0x3287360);
    ...
    auto names=global(0x2a275c8);
    unsigned nameCount=read<unsigned>(reinterpret_cast<void*>(base+0x2a275d4));
    ...
    for(unsigned index=0;index<count;++index) {
        ...
        if(kind=="planet") {
            auto candidate=engine<Byte*(*)(Byte*)>(0x39a5b0)(entry);
...
static Byte* definition(const std::string& name,const std::string& kind) {
    auto manager=global(0x339afe8);
    if(!manager) throw std::runtime_error("event-manager-unavailable");
    NativeString key(name);
    unsigned id=engine<unsigned(*)(void*,void*)>(0x406270)(manager,key.bytes);
    auto handle=engine<Byte*(*)(void*,unsigned)>(0x4064f0)(manager,id);
...
static Byte& randomForbidden() { return *reinterpret_cast<Byte*>(base+0x299f064); }
struct NativeRandomPermission {
    Byte previous=randomForbidden();
    // Native console event entry 0xf4349a/0xf434a5 and restoration 0xf43e61.
...
struct NativeScope {
    ...
    NativeScope(const std::string& kind,Byte* object) {
        auto seed=engine<unsigned(*)(const char*,int)>(0x1bed960)("sdk449_bridge",1);
        engine<void(*)(void*,unsigned)>(0x398160)(bytes,seed);
        engine<void(*)(void*,void*)>(kind=="country"?0x395ee0:0x395c80)(bytes,object);
    }
    ~NativeScope() { engine<void(*)(void*)>(0x23c5b0)(bytes); }
};
static bool condition(const Json& script,Byte* object) {
    ...
    return engine<bool(*)(void*,void*)>(0x941e10)(event,scope.bytes);
}
...
static Json invoke(const Json& command,const Json& invocation,Binding& binding,Json& evidence,Json& raw) {
    ...
    if(script["kind"]=="condition") {
        capture("start");
        bool observed=engine<bool(*)(void*,void*)>(0x941e10)(event,scope.bytes);
        capture(observed?"result:true":"result:false"); capture("end"); value=observed;
    } else {
        engine<void(*)(void*,void*,int,void*)>(0x942570)(event,scope.bytes,0,nullptr);
...
static int save(const std::string& name) {
    auto idler=global(0x3287900);
    auto manager=idler?read<Byte*>(idler,0xf88):nullptr;
    if(!manager || read<Byte>(manager,0x68)) throw std::runtime_error("save-unavailable-or-pending");
    NativeString key(name);
    return engine<int(*)(void*,void*,bool,bool)>(0xaf52c0)(manager,key.bytes,false,false);
}
static bool normalBoundary() {
    void* frames[64]; auto count=CaptureStackBackTrace(0,64,frames,nullptr);
    for(unsigned i=0;i<count;++i) if(reinterpret_cast<uintptr_t>(frames[i])==base+0x1bba8ca) {
        static bool captured=false;
        if(!captured) {
            captured=true;
            Json stack=Json::array(); for(unsigned j=0;j<count;++j) stack.push_back(reinterpret_cast<uintptr_t>(frames[j])-base);
            writeJson("normal-boundary.json",{{"rva",0x1bba8ca},{"stack",stack},{"tid",GetCurrentThreadId()}});
        }
        return true;
    }
    return false;
...
static Json process(const Json& request,Json& raw) {
    ...
    if(action=="inspect") return {{"snapshot",before},{"targets",targets()},{"deaths",deaths},{"savePending",read<Byte>(read<Byte*>(global(0x3287900),0xf88),0x68)!=0}};
    if(action=="ai-off") {
        if(before["ai"]=="on") {
            alignas(16) Byte output[64]={0};
            engine<void(*)(void*,void*,void*)>(0xf416d0)(output,nullptr,nullptr);
            raw["toggleText"]=nativeString(output+8);
            engine<void(*)(void*)>(0x160670)(output+8);
        }
    ...
    if(command["kind"]=="snapshot") outcome=completed(before);
        ...
        engine<void(*)(int,bool)>(0xa80680)(days,false);
...
static LRESULT CALLBACK messageHook(int code,WPARAM wp,LPARAM lp) {
    if(code>=0) {
        __try { poll(); }
        __except(EXCEPTION_EXECUTE_HANDLER) { nativeFault=GetExceptionCode(); faulted=true; }
    }
...
static DWORD WINAPI initialize(void*) {
    try {
        wchar_t path[4096]; GetEnvironmentVariableW(L"SDK449_NATIVE",path,4096); directory=path;
        ...
        installHook(0x721260,reinterpret_cast<void*>(destroyCountry),reinterpret_cast<void**>(&countryDestructor));
        installHook(0xe79720,reinterpret_cast<void*>(destroyPlanet),reinterpret_cast<void**>(&planetDestructor));
        installHook(0x18766b0,reinterpret_cast<void*>(logEffect),reinterpret_cast<void**>(&originalLog));
        if(MH_EnableHook(MH_ALL_HOOKS)!=MH_OK) throw std::runtime_error("hook-enable-failed");
        // The hook and target thread are in this process; no DLL lookup is required.
        messageHookHandle=SetWindowsHookExW(WH_GETMESSAGE,messageHook,nullptr,mainThread);
        if(!messageHookHandle) throw std::runtime_error("message-hook-failed:"+std::to_string(GetLastError()));
        writeJson("loaded.json",{{"pid",GetCurrentProcessId()},{"mainThread",mainThread},{"world",world},{"base",base}});
        for(;;) {
            if(auto code=nativeFault.exchange(0)) writeJson("bridge-failure.json",{{"nativeException",code}});
            PostThreadMessageW(mainThread,WM_NULL,0,0); Sleep(100);
        }
