...
/** One fixture trigger, read straight from the install. */
function trigger(name: string) {
  ...
  const found = read.definitions.find((one) => one.name === name);
  if (found === undefined) {
    throw new Error(`no fixture trigger ${name}`);
  }
  return found;
}

const readParams = (name: string) => trigger(name).params;
const readShapes = (name: string) =>
  trigger(name).shapes.map((shape) =>
    Object.fromEntries(shape.params.map(({ name: key, presence }) => [key, presence]))
  );
...
describe("registry readers", () => {
  ...
  it("reads anonymous repeated definitions without inventing a vanilla id", () => {
    const root = mkdtempSync(path.join(tmpdir(), "pdx-anonymous-registry-"));
    const directory = path.join(root, "common/terraform");
    mkdirSync(directory, { recursive: true });
    writeFileSync(
      path.join(directory, "00_links.txt"),
      "terraform_link = { from = pc_ocean to = pc_gaia duration = 1800 }\n"
    );

    try {
      const read = readRegistryIds(root, {
      ...
      expect(read).toMatchObject({ ids: [], files: 1, missing: false });
    } finally {
      rmSync(root, { recursive: true, force: true });
    }
  });

  it("reads country types from their top-level keys", () => {
    expect(file("registries/country-type.ts")).toMatchInlineSnapshot(`
      "// Generated by @pdx-ts/codegen-vanilla from Stellaris 4.4.6. Do not edit.

      /** Identifiers defined in Stellaris 4.4.6 for the \`country_type\` registry. */
      export type VanillaCountryTypeId = \"guardian\";
      "
    `);
  });
...
describe("scripted parameters", () => {
  it("emits one flat object per definition, and a union only where choices disagree", () => {
    expect(file("scripted-triggers.ts")).toMatchInlineSnapshot(`
      ...
      export interface VanillaScriptedTriggerParams {
        ...
        readonly fake_inert_region: {
          readonly TECH: string | number;
          readonly TOGGLE?: string | number;
        };
        readonly fake_mandatory_flag: {
          readonly GATE: string | number;
          readonly INNER: string | number;
        };
        readonly fake_mixed_default: {
          readonly YEARS: string | number;
        };
        readonly fake_negated_pair: {
          readonly NAME: string | number;
          readonly TOGGLE?: string | number;
        };
  ...
  it("keeps a name required when only some of its occurrences carry a default", () => {
    // `$YEARS|10$` in one place and `$YEARS$` in another. The second
    // substitution has nothing to fall back on, so omitting the parameter
    // leaves a `$YEARS$` in the emitted script with nothing to substitute.
    // Reading one defaulted occurrence as permission to omit the name made the
    // signature accept exactly that call.
    expect(readParams("fake_mixed_default")).toEqual([{ name: "YEARS", optional: false }]);
  });

  it("keeps the caller's choices apart when they disagree", () => {
    // `[[FLAG] ... $NAME$ ... ]` with no negated twin is a choice, not two
    // independent optional names: omitting `FLAG` never reaches `$NAME$`, and
    // supplying it leaves nothing to substitute unless `NAME` comes too.
    expect(readShapes("fake_conditional")).toEqual([
      { FLAG: "forbidden", NAME: "forbidden" },
      { FLAG: "required", NAME: "required" },
    ]);
  });

  it("collapses to one shape when the choices agree", () => {
    // The region substitutes only `$TECH$`, which the body already requires
    // outside it. Branching would double the call shapes and constrain nothing.
    expect(readShapes("fake_inert_region")).toEqual([{ TECH: "required", TOGGLE: "optional" }]);
  });

  it("reads a negated region, so a name both branches substitute stays required", () => {
    // The shape vanilla actually writes. `[[X] ... $N$ ... ]` beside
    // `[[!X] ... $N$ ... ]` means exactly one branch always runs, so `$N$` is
    // always substituted: `N` is required and `X` is an ordinary optional flag.
    // Treating every region as presence-activated made this look like a
    // dependency and published a type that refused `{ N: "..." }`.
    expect(readShapes("fake_negated_pair")).toEqual([{ NAME: "required", TOGGLE: "optional" }]);
  });

  it("requires what a mandatory flag's region reaches", () => {
    // `$GATE$` is substituted outside every region, so the caller must supply
    // it, so `[[GATE] ... ]` is always active and what it substitutes is
    // required too. The choice that omits `GATE` describes a call nobody can
    // make and is dropped.
    expect(readShapes("fake_mandatory_flag")).toEqual([{ GATE: "required", INNER: "required" }]);
  });

  it("refuses a call that activates a region without its parameters", () => {
    const emitted = file("scripted-triggers.ts");

    // Region off: neither name may be passed. Region on: both must be.
    expect(emitted).toContain("readonly FLAG?: never;");
    expect(emitted).toContain("readonly NAME?: never;");
    expect(emitted).toContain("readonly FLAG: string | number;");
    expect(emitted).toContain("readonly NAME: string | number;");
    expect(emitted).not.toContain("readonly NAME?: string | number;");
  });
  ...
  it("counts definitions, parameterized definitions, and region-dependent ones", () => {
    expect(generated.report.scripted).toMatchObject([
      {
        registry: "scripted_trigger",
        definitions: 10,
        parameterized: 7,
        regionDependent: 1,
        files: 1,
...
describe("report", () => {
  it("distinguishes anonymous registries from a failed id lookup", () => {
    expect(registryReport("terraform_link")).toMatchObject({ anonymous: true, ids: 0 });
  });
