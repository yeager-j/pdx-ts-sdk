...
const readRegions = (name: string) => trigger(name).regions;
...
describe("scripted parameters", () => {
  ...
  it("ties a region's parameters to the region rather than listing them beside it", () => {
    // `[[FLAG] ... $NAME$ ... ]` is a choice, not two independent optional
    // names: `{ FLAG: true }` alone activates the region and leaves `$NAME$`
    // unsubstituted.
    expect(readRegions("fake_conditional")).toEqual([{ condition: "FLAG", requires: ["NAME"] }]);
  });

  it("records no region when activating it forces nothing new", () => {
    // The region substitutes only `$TECH$`, which the body already requires
    // outside it. Branching on the region would double the call shapes and
    // constrain nothing.
    expect(readRegions("fake_inert_region")).toEqual([]);
    expect(file("scripted-triggers.ts")).toContain("readonly TOGGLE?: string | number;");
  });
