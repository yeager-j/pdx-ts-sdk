 ...
 *
 * Highlighting in the browser would mean shipping TextMate grammars and an
 * oniguruma wasm blob to render text that never changes — several times the
 * weight of the page itself, to colour a snapshot that was fixed when it was
 * committed. So Shiki runs here, in Node, and the viewer receives HTML.
 *
 * The result is a *derived viewer asset*, not part of the Reference build: it
 * goes into the bundle through a virtual module and never into
 * `data/situation-reference.json`. Two reasons. The committed snapshot is a
 * review surface, and a diff full of `<span style="--shiki-light:#…">` is a
 * diff nobody can read. And presentation is not a claim — the same build
 * rendered with different colours is the same build.
 *
 * Both themes are baked into one pass. Shiki emits `--shiki-dark` custom
 * properties alongside the light colours, so a single HTML string serves both
 * schemes and the page keeps working the way the rest of it does, off
 * `prefers-color-scheme` with no JavaScript.
 */
 ...
 *
 * Shiki has no Paradox grammar, and the output half of a story panel is where a
 * reader who knows TypeScript and not Stellaris learns what their code becomes.
 * Leaving it flat grey blunts the most useful thing on the page.
 *
 ...
 * round-trips the whole language, and duplicating its knowledge here would be a
 * second grammar to keep in step.
 *
 * Two constructs are deliberately absent rather than overlooked: conditional
 * blocks (`[[PARAM] … ]`) and inline math (`@[ … ]`). Neither appears anywhere
 * in this page's output, both need nested states, and an unexercised rule is an
 * unverified one.
 */
const PDXSCRIPT: LanguageRegistration = {
  ...
  repository: {
     ...
     * Stellaris 4.4. There is deliberately no `==`: the lexer reads that as two
     * separate `=` tokens, and inventing it here would be inventing syntax.
     */
     ...
     *
     * A hard-coded list, because resolving these properly means reading the CWT
     * link table, which is a rule database rather than a grammar. These seven
     * are the ones Stellaris script actually writes.
     */
    prefixedValue: {
 ...
 *
 * `tide_swell_calm:0 "Calm"` has a version number between the key and the
 * value, and YAML's grammar cannot see past it — it reads the whole line as one
 * key and colours the English text the same as the generated id. That is
 * exactly the distinction this panel exists to show, since the page's claim is
 * that you write the text and the build produces the key.
 */
 ...
 * The theme pair.
 *
 * Not a free choice: the theme decides how much of the grammar a reader can
 * actually see. GitHub's themes render keys, numbers and booleans in one
 * identical blue, collapsing fourteen distinct token scopes into four visible
 * colours — the grammar is still right, and the page still looks flat.
 * Catppuccin separates the trio that carries the meaning here (key, value,
 * number) and stays muted enough that the output panel is not the loudest
 * thing on a neutral page.
 */
export const THEMES = { light: "catppuccin-latte", dark: "catppuccin-mocha" } as const;

/** Which grammar an output file gets, by extension. */
...
export interface HighlightedStory {
  /** The story's source, as HTML. */
  readonly code: string;
  /** Each output file, as HTML, keyed by the same paths the snapshot uses. */
...
export async function highlightStories(
  stories: readonly StoryLike[]
): Promise<Record<string, HighlightedStory>> {
  const highlighter = await createHighlighter({
    themes: [THEMES.light, THEMES.dark],
    langs: ["typescript", PDXSCRIPT, PDXLOC],
  });
  const highlighted: Record<string, HighlightedStory> = {};
  for (const story of stories) {
    const output: Record<string, string> = {};
    for (const [path, text] of Object.entries(story.output)) {
      output[path] = highlighter.codeToHtml(text, { lang: languageOf(path), themes: THEMES });
    }
    highlighted[story.id] = {
      code: highlighter.codeToHtml(story.code, { lang: "typescript", themes: THEMES }),
      output,
    };
  }
  highlighter.dispose();
