 ...
 *
 * Scopes are asserted, not colours. A theme change is a taste change and should
 * not break a test; the grammar deciding that a version string is a number is a
 * correctness change and should.
 */
...
describe("keys are decided by position, not by shape", () => {
  ...
  it("includes keys that are not identifiers", () => {
    // An identifier-shaped rule drops all three of these, which leaves holes at
    // the starts of lines and reads as a broken highlighter.
...
describe("operators", () => {
  ...
  it("treats `==` as two operators rather than inventing a form", () => {
    // The language has no `==` — the plugin's lexer reads it as two `=`
    // tokens. So the grammar matches `=` twice and Shiki merges the pair into
    // one span, which is why a reader sees a fully coloured `==` regardless.
...
describe("localization is not YAML", () => {
  it("separates the generated key, the version, and the English text", () => {
    const line = 'l_english:\n tide_swell_calm:0 "Calm"';
    expect(scope(line, "tide_swell_calm", "pdxloc")).toBe("support.type.property-name");
    expect(scope(line, ":0", "pdxloc")).toBe("keyword.operator");
    expect(scope(line, '"Calm"', "pdxloc")).toBe("string.quoted.double");
  });
});
