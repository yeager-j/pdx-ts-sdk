/**
 * One ship event, and the feature that places it.
 *
 ...
 *
 * The definer is what makes this a `ship_event`. There is no `type`
 * field to set: `events.ship(...)` decides both the kind the game reads and the
 * scope every callback below is handed, so the event happens to a ship and the
 * effects legal inside it are ship-scope effects. Firing it from elsewhere goes
 * through `shipEvent`.
 *
...
// method on the scope you are in: `<scope>.shipEvent({ id: resonanceTheory })`.
// The `research-quest` recipe generates one wired end to end.
export const resonanceTheory = events.ship(1, {
  ...
  options: [
    {
      ...
      effects: (ship) => {
        ...
        ship.setShipFlag(`${mod.config.prefix}_resonance_theory_fired`);
      },
    },
  ],
