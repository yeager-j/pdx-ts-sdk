...
| --- | --- | --- |
| `@pdx-ts/sdk` | The pipeline a build script calls | `createMod`, `discoverFeatures`, `render`, `write`, `install`, `runBuild`, `runInstall`, the error classes |
...
| `@pdx-ts/sdk/reference` | Machine-readable facts about the SDK, for tools that reason about the surface | `CONTENT_REGISTRIES`, `SCRIPT_REFERENCE_SCOPES`, `EVENT_KINDS`, `SUPPORTED_STELLARIS_BUILD` |
