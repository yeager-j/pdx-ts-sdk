...
import { reconcileModifierScopes, type CanonicalScope } from "../../reconcile/modifier-scopes.ts";
...
export function joinModifierScopes(
  ...
  const joined = reconcileModifierScopes(rules, docs, canonical);
  return {
    ...joined,
    dynamicFamilies: dynamicModifierFamilies(rules, joined.categoryScopes),
    categoryScopes: new Map(
      [...joined.categoryScopes].map(([category, scopes]) => [
        category,
...
function economicSelectorCode(): string {
  ...
  return (
    ...
    'export type EconomicTriggeredRows<W extends import("../content/types.ts").EconomicCategoryWitness, F extends EconomicTriggeredField> = W[EconomicTriggeredWitnessField<F>] extends readonly (infer R)[] ? R : never;\n' +
    "export type EconomicTriggeredKeyId<K> = K extends string ? K : K extends { readonly id: infer I extends string } ? I : never;\n" +
    "export type EconomicTriggeredRowKey<R> = R extends { readonly key: infer K } ? EconomicTriggeredKeyId<K> : never;\n" +
    "export type EconomicTriggeredKeyOfRow<R> = R extends { readonly key: infer K } ? EconomicTriggeredKeyId<K> extends infer I extends string ? string extends I ? never : I : never : never;\n" +
    'export type EconomicTriggeredKeys<W extends import("../content/types.ts").EconomicCategoryWitness> = { [F in EconomicTriggeredField]: EconomicTriggeredKeyOfRow<EconomicTriggeredRows<W, F>> }[EconomicTriggeredField];\n' +
    'export type EconomicTriggeredRowModifierTypes<R> = R extends { readonly modifierTypes: infer M } ? M extends readonly ("add" | "mult")[] ? number extends M["length"] ? never : M[number] & ("add" | "mult") : never : never;\n' +
    'export type EconomicTriggeredWidenedKeyOfRow<R> = R extends { readonly key: infer K; readonly modifierTypes: infer M } ? M extends readonly ("add" | "mult")[] ? number extends M["length"] ? EconomicTriggeredKeyId<K> : never : EconomicTriggeredKeyId<K> : never;\n' +
    'export type EconomicTriggeredWidenedKeys<W extends import("../content/types.ts").EconomicCategoryWitness> = { [F in EconomicTriggeredField]: EconomicTriggeredWidenedKeyOfRow<EconomicTriggeredRows<W, F>> }[EconomicTriggeredField];\n' +
    'export type EconomicTriggeredTypes<W extends import("../content/types.ts").EconomicCategoryWitness, F extends EconomicTriggeredField, K extends string> = EconomicTriggeredRows<W, F> extends infer R ? R extends unknown ? K extends EconomicTriggeredRowKey<R> ? EconomicTriggeredRowModifierTypes<R> : never : never : never;\n' +
    'export type EconomicTriggeredCapability<W extends import("../content/types.ts").EconomicCategoryWitness, K extends string> = K extends unknown ? { [F in EconomicTriggeredField]: EconomicTriggeredTypes<W, F, K> } : never;\n' +
    'export type EconomicTriggeredKeyGuard<W extends import("../content/types.ts").EconomicCategoryWitness, K> = EconomicTriggeredKeyId<K> extends infer I extends string ? string extends I ? never : [I] extends [EconomicTriggeredKeys<W>] ? [Extract<I, EconomicTriggeredWidenedKeys<W>>] extends [never] ? IsUnion<EconomicTriggeredCapability<W, I>> extends true ? never : K : never : never : never;\n' +
    'export type EconomicTriggeredResourceRecorder<W extends import("../content/types.ts").EconomicCategoryWitness, K extends string> = { [F in EconomicTriggeredField as "add" extends EconomicTriggeredTypes<W, F, K> ? F : never]: { readonly add: ModifierSetter } } & { [F in EconomicTriggeredField as "mult" extends EconomicTriggeredTypes<W, F, K> ? F : never]: { readonly mult: ModifierSetter } };\n' +
