/**
 * Registry-typed collection factories (SDK-22). Each factory is a collection
 * bound to one output file and returns only the definers its registry
 * allows; the definers register what they create, so creation IS
 * registration. `createEvents` co-declares file and namespace at one site —
 * one namespace per event file holds by construction, visible while
 * authoring, not discovered at build.
 *
 * `makeCollection`, `createEvents`, `createOnActions` and the
 * `defineSituationType` graft wrapper are the permanently hand-written
 * parts. The plain content factories below (`createTechnologies`,
 * `createSituationTypes`, `createCountryShipOfSizeLimits`) are interim
 * hand-written stand-ins so the pure API has evidence to run against;
 * codegen emits all 34 of them — one per registry, exactly where the
 * `GeneratedContentMethods` class methods are emitted today — and replaces
 * these. The pluralized naming needs a codegen rule (`technology` →
 * `createTechnologies`).
 */

import { buildEvent, type DefinedEvent, type EventDef } from "./events.ts";
import type { CountryShipOfSizeLimitDef } from "./generated/country-ship-of-size-limit.ts";
...
import type { TechnologyDef } from "./generated/technology.ts";
import {
  assertFileStem,
  assertNamespace,
  type Collection,
